...
class WxPayConf_pub(object):
    ...
    APPID = 'wx307bbb641dbacf6d'
    APPSECRET = 'fb1f5c7739f1524fd1ca1025c06b1f0a'
    #受理商ID，身份标识
    MCHID = "1282243201"
    #商户支付密钥Key
    KEY = "tannianlaizongjiheshikuyanliu521"
...
def test():
    ...
    #c2 = HttpClient()
    #assert id(c) == id(c2)
    detail = UnifiedOrder_pub()
    detail.setParameter("out_trade_no", datetime.datetime.now().strftime('%Y%m%d%H%M%S%f'))
    detail.setParameter("body", "test")
    detail.setParameter("total_fee", "1")
    detail.setParameter("notify_url", "1")
    detail.setParameter("trade_type", "JSAPI")
    detail.setParameter("openid", "oXP2qt4NT-izUpr_B86wbViypiqI")

    print detail.getPrepayId()
